...
static float temperature; // temp sensor
 ...
 */
  ...
  imu_readback.heading_m = atan2(magValue.x, magValue.y) * 180 / PI;
  Serial.print(imu_readback.heading_m);
...
void draw_heading() {
  ...
  u8g2.setCursor(90, 0);
  u8g2.print(String(imu_readback.heading_m, 1));
}
...
void draw_temp() {
  ...
  // u8g2.print(String(temp,1)); // MPU temp
...
//       if ((gps.date.isValid()) && (gps.location.age()<1000)) 
       if (gps.date.isValid())
        { 
          ...
          // setTime(Hour, Minute, Second, Day, Month, Year); // NOTE: what is this function??
          // add the offset to get local time
          // adjustTime(AWST_TIME_OFFSET); // NOTE: what is this function??
          if (Month < 10) {
